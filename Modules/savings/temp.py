...
Clarke_Wright_Savings(model.saving, model.graph.nodes, CAPA, model.depot)

def Clarke_Wright_Savings(saving_dict, node_list, capa, depot_idx):
    """
    saving_dict: 정렬된 saving의 dictionary 객체
    node_list: 각 node들의 demand list를 나열 -> 그냥 node list를 넣고, depot_idx로 해당 node를 빼주자
    capa: 트럭의 capacity
    depot_idx: depot의 index -> 오로지 마지막에 depot index를 붙여줄 때에만 쓰임
    """
    ...
    route_list = []
    node_state_list = [[0, 0, 0] for i in range(len(node_list))]
    for link in tqdm(saving_dict):
        ...
        # 각 node의 demand
        i_demand = node_list[i_idx-1]
        j_demand = node_list[j_idx-1]
            ...
            # 기존 route들에 i, j 둘 다 있는 경우
                ...
                if (i_route_idx != j_route_idx): 
                    ...
                    for i in route_list[i_route_idx]:
                        demand += node_list[i-1]
                    for j in route_list[j_route_idx]:
                        demand += node_list[j-1]
